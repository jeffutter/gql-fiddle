...
functions/
  api/
    ...
    ws/
      index.ts        GET /api/ws — WebSocket upgrade → UserSyncDO (authenticated)
  _lib/
    ...
    UserSyncDO.ts     Durable Object — holds WebSocket connections per user; broadcasts invalidation signals
  __tests__/
    ...
    workspaces.test.ts Unit tests for workspace REST API (incl. broadcast-on-write)
    userSync.test.ts  Unit tests for UserSyncDO
...
wrangler.jsonc        Pages project config (D1 + KV + Durable Objects bindings)
...
| Durable Objects requests | 1,000,000 / month | One WS open + one broadcast per accepted write |
...
comfortably fit within the free tier for personal use.  The DO hibernatable
WebSocket API avoids billing CPU time while sockets are idle, so Durable Object
CPU usage is effectively zero between writes.
...
### Real-time sync via Durable Objects (optional — TASK-88.8)

Near-instant cross-device propagation layered on top of the pull-based strategy.

**Architecture:**

- Each logged-in user has a **per-user Durable Object** (`UserSyncDO`, keyed by
  `userId`, declared in `functions/_lib/UserSyncDO.ts`). The DO holds all open
  WebSocket connections for that user's devices using the **hibernatable WebSocket
  API** (`state.acceptWebSocket`) — no CPU is billed while sockets are idle.
- The `GET /api/ws` endpoint (`functions/api/ws/index.ts`) authenticates the
  caller via the existing session cookie, then forwards the WebSocket upgrade to
  the user's DO instance.
- After an **accepted** `PUT` or `DELETE` to the workspace API, the handler fires
  a fire-and-forget broadcast via `ctx.waitUntil()` to the user's DO, which fans
  a lightweight `{ changedId, version }` signal to all other connected sockets.
  The broadcast is best-effort — a failure never affects the API response.
- On receiving a signal, the client resets the throttle and calls
  `deltaRefresh(force=true)`. **No state travels over the WebSocket wire** — the
  full reconciliation uses the existing `?since` pull and merge logic.

**Graceful degradation:** if the DO or WebSocket path is unavailable at any
point, the app behaves exactly as in TASK-88.7 (focus/visibility pulls + 60 s
polling). The WebSocket client reconnects with exponential backoff (1 s → 60 s
cap) on unexpected disconnection.

**wrangler.jsonc additions** (see file for full config):

```jsonc
"durable_objects": {
  "bindings": [{ "name": "USER_SYNC", "class_name": "UserSyncDO" }]
},
"migrations": [{ "tag": "v1", "new_classes": ["UserSyncDO"] }]
```

Run `wrangler types` after any binding change to regenerate
