...
| `0003_users_kwk.sql` | Adds `kwk TEXT` column to `users` (moves the KWK out of KV so its first-write is race-safe; see TASK-118). Left the column unbackfilled for existing accounts, orphaning any `wrapped_dek` set before this migration shipped — recovered via the legacy-KV `confirm_kwk` flow, see TASK-128 |
...
functions/
  api/
    ...
    auth/
      ...
      enc-meta.ts     GET /api/auth/enc-meta — returns { kwk, wrapped_dek, legacy_kwk } for the session user
                      PUT /api/auth/enc-meta — stores the client-generated wrapped DEK, and/or
                        confirms a legacy KWK recovered from KV (confirm_kwk, see TASK-128)
    workspaces/
  ...
  _lib/
    db.ts             D1 data-access helpers (getOrCreateUser, listWorkspaces, upsertWorkspace,
                        softDeleteWorkspace, getWrappedDek, setWrappedDek, getKwk,
                        setKwkIfAbsent, setKwk — unconditional overwrite for the TASK-128
                        legacy-KWK repair path)
...
migrations/
  ...
  0003_users_kwk.sql  Adds kwk column to users (KWK moved from KV to D1; unbackfilled,
                        see TASK-128 for the legacy-KV recovery path this required)
...
Workspace payloads are encrypted before being sent to the server so that the
plaintext DEK is never transmitted to or stored on the server. Implementation
lives in `web/src/encryption.ts`.

**Two-layer key system:**

- **KWK (Key Wrapping Key)** — a random 256-bit key generated server-side and
  stored in D1 (the `users.kwk` column added in migration 0003). Never leaves
  the server. Originally lived in KV, but that made first-login creation
  racy (KV has no compare-and-swap and is only eventually consistent); it
  moved to D1 to reuse the same race-safe conditional-write pattern as
  `wrapped_dek` (see TASK-118).
...
The KWK and the wrapped DEK now both live in D1 — this is no longer a
two-separate-storage-backends design. The security property that remains is
that the plaintext DEK is generated and used only in the browser and is never
reconstructed server-side, so a D1 compromise yields ciphertext (the wrapped
DEK) and the KWK, but not the unwrapped DEK.

**Key bootstrap flow (`initEncryption`, called on login):**

1. `GET /api/auth/enc-meta` → receives the KWK, `wrapped_dek`, and
   `legacy_kwk` (all derived from D1 plus a KV read; `wrapped_dek` may be
   null on first login; `legacy_kwk` is normally null — see recovery below).
2. If `wrapped_dek` is present, decrypt it with the KWK to recover the DEK.
   If that fails and the server offered a `legacy_kwk`, retry the unwrap
   with it (see below) before giving up.
...
**Legacy KWK recovery (TASK-128):** migration 0003 (TASK-118) added the D1
`kwk` column with no backfill from the pre-existing `kwk:<user_id>` KV
entries it replaced. Any account that had already completed key setup
before TASK-118 shipped got an unrelated, freshly-generated D1 `kwk` on its
next login, permanently orphaning its existing `wrapped_dek` (no key could
ever unwrap it again). Because the server never sees the plaintext DEK, it
cannot tell an orphaned account apart from one whose stale legacy KV entry
was already superseded — only the browser can prove which KWK is correct,
by attempting the decrypt. So `GET /api/auth/enc-meta` offers the legacy KV
value as `legacy_kwk` whenever it disagrees with the stored D1 `kwk` and a
`wrapped_dek` already exists; if `web/src/encryption.ts` unwraps
successfully with it, it adopts the resulting DEK locally and calls
`PUT /api/auth/enc-meta` with `{ confirm_kwk: legacy_kwk }` so the server
persists the confirmed KWK (via `setKwk`, an unconditional overwrite) and
future logins on any device skip the fallback. The server re-verifies
`confirm_kwk` against its own KV read before trusting it — it never adopts
...
functions/
  api/
    ...
    auth/
      ...
      enc-meta.ts   GET/PUT /api/auth/enc-meta (KWK + wrapped DEK exchange, plus
                      legacy-KV KWK recovery/confirm_kwk — TASK-128)
  _lib/
...
migrations/
  ...
  0003_users_kwk.sql  kwk column (KWK moved from KV to D1, see TASK-118; unbackfilled,
                        orphaning pre-existing wrapped_dek rows — recovered per TASK-128)
