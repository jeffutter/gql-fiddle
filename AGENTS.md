...
migrations/
  ...
  0003_users_kwk.sql  Adds kwk column to users (KWK moved from KV to D1)
...
1. `GET /api/auth/enc-meta` → receives the KWK and `wrapped_dek` (both from
   D1; `wrapped_dek` may be null on first login).
...
migrations/
  ...
  0003_users_kwk.sql  kwk column (KWK moved from KV to D1, see TASK-118)
